...
class Base:
    ...
    def __init__(
        ...
        logger.info(f"Initialised: {self.__class__.__name__}" f"('{self.country}', '{self.app_name}', {self.app_id})")
    ...
    def _landing_url(self):
        ...
        return landing_url.format(country=self.country, app_name=self.app_name, app_id=self.app_id)
    ...
    def _get(
        ...
        with catch_warnings(), requests.Session() as s:
    ...
    def _log_status(self):
        logger.info(
            f"[id:{self.app_id}] Fetched {self._fetched_count} reviews " f"({self.reviews_count} fetched in total)"
        )
